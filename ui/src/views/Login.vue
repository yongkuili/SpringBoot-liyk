...
import {getKaptcha} from "../http/modules/login";
export default {
  name: 'Login',
  components:{
    ThemePicker
  ...
  methods: {
    login() {
      this.loading = true
      let userInfo = {account:this.loginForm.account, password:this.loginForm.password, captcha:this.loginForm.captcha,unique:this.loginForm.unique}
    ...
    refreshCaptcha: function(){
      this.$api.login.getKaptcha().then((res) => {
        console.log(res)
        this.loginForm.src = res.code;
        this.loginForm.unique = res.key;
      }).catch((res) => {
        this.$message({
          message: res.message,
          type: 'error'
